...
export default function KineticOverlay() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-white flex flex-col">
      <header className="flex items-center bg-[#0b0f19] border-b border-emerald-500/10 p-4">
        ...
        <div className="ml-auto">
          <button className="flex size-10 items-center justify-center rounded-full hover:bg-emerald-500/10">
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex items-end gap-3 ${msg.role === "user" ? "justify-end" : ""}`}>
            {msg.role === "ai" && (
              <div className="size-8 shrink-0 rounded-full bg-gradient-to-br from-emerald-800 to-emerald-500 flex items-center justify-center shadow-lg">
            ...
            <div className={`flex flex-col gap-1 ${msg.role === "user" ? "items-end" : "items-start"} max-w-[85%]`}>
              ...
              <div className={`rounded-xl px-4 py-3 ${msg.role === "user" ? "bg-gradient-to-br from-emerald-800 to-emerald-500 text-white rounded-tr-none" : "bg-[#1f2a3d66] text-slate-100 border border-emerald-500/20 rounded-tl-none"} shadow-lg`}>
                <p>{msg.content}</p>
              </div>
              {msg.time && <p className="text-[10px] text-slate-500 mt-1">{msg.time}</p>}
            </div>
            {msg.role === "user" && (
              <div className="size-8 shrink-0 rounded-full bg-slate-800 overflow-hidden border border-emerald-500/20">
      ...
      <div className="p-4 bg-[#0b0f19] border-t border-emerald-500/10 shrink-0">
        <div className="flex items-center gap-2 bg-slate-800/50 rounded-full px-4 py-2 border border-emerald-500/10">
          ...
          />
          <button onClick={handleSend} className="size-8 rounded-full bg-gradient-to-br from-emerald-800 to-emerald-500 flex items-center justify-center text-white">
            <span className="material-symbols-outlined font-bold">arrow_upward</span>
          </button>
        </div>
      </div>

      <nav className="flex gap-2 border-t border-emerald-500/10 bg-[#0b0f19] p-4 pb-8">
        <a className="flex flex-1 flex-col items-center justify-center gap-1 text-emerald-400" href="#" onClick={(e) => { e.preventDefault(); navigate("/dashboard"); }}>
