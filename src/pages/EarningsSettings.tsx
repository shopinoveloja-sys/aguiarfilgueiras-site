...
export default function EarningsSettings() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-white pb-24">
      <header className="fixed top-0 w-full z-50 bg-[#0b0f19]/80 backdrop-blur-md border-b border-emerald-500/10 flex items-center justify-between px-6 py-4">
      ...
      <main className="pt-20 px-4 max-w-2xl mx-auto space-y-6">
        <section>
          ...
          <div className="grid grid-cols-2 gap-3">
            {["Uber", "99", "InDrive", "Maxim"].map((app) => (
              <div key={app} className="bg-[#1f2a3d66] rounded-xl border border-emerald-500/10 p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                ...
                <div className="w-6 h-6 rounded-md border-2 border-emerald-500 bg-emerald-500 flex items-center justify-center">
        ...
        <section className="space-y-4">
          ...
          <div className="bg-[#1f2a3d66] rounded-xl border border-emerald-500/10 divide-y divide-emerald-500/10">
            {[
              ...
              <div key={item.key} className="p-4 flex items-center justify-between">
                ...
                <button
                  ...
                  className={`w-12 h-6 rounded-full relative p-1 cursor-pointer transition-colors ${settings[item.key as keyof typeof settings] ? "bg-emerald-500" : "bg-slate-700"}`}
                >
        ...
        <button className="w-full py-4 bg-gradient-to-br from-emerald-800 to-emerald-500 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-all duration-200 mt-8 mb-4">
          Salvar Configurações
        </button>
      </main>

      <nav className="fixed bottom-0 left-0 w-full flex justify-around items-center px-4 py-3 pb-8 bg-[#0b0f19]/80 backdrop-blur-md z-50 border-t border-emerald-500/10">
        <div className="flex flex-col items-center justify-center text-slate-500 hover:text-emerald-400" onClick={() => navigate("/dashboard")}>
          <span className="material-symbols-outlined">dashboard</span>
          <span className="text-[10px] font-bold uppercase tracking-wider">Dashboard</span>
        </div>
        <div className="flex flex-col items-center justify-center text-slate-500 hover:text-emerald-400" onClick={() => navigate("/rides")}>
          <span className="material-symbols-outlined">directions_car</span>
          <span className="text-[10px] font-bold uppercase tracking-wider">Rides</span>
        </div>
        <div className="flex flex-col items-center justify-center text-slate-500 hover:text-emerald-400">
          <span className="material-symbols-outlined">insights</span>
          <span className="text-[10px] font-bold uppercase tracking-wider">Insights</span>
        </div>
        <div className="flex flex-col items-center justify-center text-emerald-500 bg-emerald-500/10 rounded-xl px-3 py-1" onClick={() => navigate("/profile")}>
